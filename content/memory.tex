...
Scoped pointers can be implicitly casted to raw. All other managed pointer must be force casted to a raw pointer by using \tcode{rawptr()}. The \tcode{rawptr} function should be avoided and is only available for backwards compatability with legacy C++ systems.
